...
export default function SportsPaymentSuccess() {
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl p-6 space-y-6">
        <div className="flex flex-col items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-gray-800 text-center">Payment Successful!</h1>
          <p className="text-gray-600 text-center">Your tickets for 
            <span className="font-semibold">{event.title}</span> have been booked.</p>
        </div>
        {event.img && (
          <div className="flex justify-center">
            <img src={event.img} alt={event.title} className="w-40 h-60 object-cover rounded-lg shadow-md" />
          </div>
        )}
